  ...
  return (
    <header className="flex items-center justify-between gap-4 border-b-2 border-line bg-white px-5 py-4 sm:px-8">
      ...
      <div className="hidden items-center gap-3 sm:flex">
        <button type="button" className="relative rounded-xl border-2 border-line p-2.5 text-slate" aria-label="Avisos">
    ...
    <>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1.3fr_1fr]">
        ...
        <section className="rounded-2xl border-2 border-line bg-white p-6 sm:p-7">
          <div className="flex items-center justify-between"><h2 className="font-display text-xl font-extrabold">Suas turmas</h2><button type="button" className="text-sm font-extrabold text-violet">Ver todas</button></div>
...
function GestaoVisao() {
  return (
    <>
      ...
      <section className="mt-6 rounded-2xl border-2 border-line bg-white p-6 sm:p-7"><div className="flex items-center justify-between"><div><h2 className="font-display text-xl font-extrabold">Progresso dos alunos · 9º A</h2><p className="mt-1 text-sm font-bold text-slate">Visão agregada por participação</p></div><button type="button" className="hidden items-center gap-2 rounded-xl border-2 border-line px-4 py-2.5 text-sm font-extrabold text-violet sm:flex">Ver relatório <ChevronRight className="size-4" /></button></div><div className="mt-5 grid gap-x-8 gap-y-4 sm:grid-cols-2">{ALUNOS.map(([nome, xp, participacao]) => <div key={nome} className="flex items-center gap-3"><span className="flex size-9 items-center justify-center rounded-full bg-violet-soft text-sm font-extrabold text-violet-deep">{nome[0]}</span><div className="min-w-0 flex-1"><div className="mb-1 flex justify-between text-sm"><span className="truncate font-extrabold">{nome}</span><span className="font-bold text-slate">{xp}</span></div><Barra valor={participacao} cor={participacao >= 75 ? 'bg-mint' : 'bg-amber'} /></div></div>)}</div></section>
    </>
  )
}
